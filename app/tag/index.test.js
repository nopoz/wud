...
describe('transform', () => {
    ...
    describe('edge cases', () => {
        test('should return original tag when formula is undefined', () => {
            expect(semver.transform(undefined, '1.2.3')).toBe('1.2.3');
        });

        test('should return original tag when formula is empty string', () => {
            expect(semver.transform('', '1.2.3')).toBe('1.2.3');
        });

        test('should return original tag when formula is invalid', () => {
            expect(semver.transform('invalid-formula', '1.2.3')).toBe('1.2.3');
        });

        test('should handle formula with no matches', () => {
            expect(semver.transform('^nomatch$ => $1', '1.2.3')).toBe('1.2.3');
        });

        test('should handle malformed regex', () => {
...
describe('integration tests', () => {
    test('should handle complete semver workflow', () => {
    ...
    test('should handle Docker-style tags', () => {
        const dockerTags = [
            'nginx:1.21',
            'nginx:1.21.6',
            'nginx:1.21.6-alpine',
